"""Generate dokumen bukti guru (PDF + PNG)"""
import random
...
def _render_template(first_name: str, last_name: str) -> str:
    """Baca template, ganti nama/ID pegawai/tanggal, dan expand variabel CSS."""
    ...
    # Ganti nama contoh / ID pegawai / tanggal (nama muncul dua tempat dalam template + span)
...
def generate_teacher_png(first_name: str, last_name: str) -> bytes:
    """Gunakan Playwright screenshot untuk generate PNG (perlu playwright + chromium sudah diinstall)."""
    try:
        from playwright.sync_api import sync_playwright
    except ImportError as exc:
        raise RuntimeError(
            "Perlu install playwright, silakan jalankan `pip install playwright` lalu `playwright install chromium`"
        ) from exc
    ...
    with sync_playwright() as p:
        ...
        page.wait_for_timeout(500)  # Biarkan style stabil
...
# Kompatibilitas panggilan lama: default generate PDF
