...
class SheerIDVerifier:
    ...
    def normalize_url(url: str) -> str:
        """Normalisasi URL (tetap original, kompatibel dengan interface yang ada)"""
        return url
    ...
    def create_verification(self) -> str:
        """Request verificationId baru melalui installPageUrl"""
        body = {
        ...
        if status != 200 or not isinstance(data, dict) or not data.get("verificationId"):
            raise Exception(f"Gagal buat verification (status {status}): {data}")

        self.verification_id = data["verificationId"]
        logger.info(f"✅ Berhasil dapat verificationId: {self.verification_id}")
    ...
    def verify(
        ...
        """Eksekusi proses verifikasi guru"""
        try:
            ...
            if not self.verification_id:
                logger.info("Request verificationId baru ...")
            ...
            # Generate PNG guru
            logger.info("Step 1/5: Generate dokumen PNG guru...")
            ...
            # Submit info guru
            logger.info("Step 2/5: Submit info guru...")
            step2_body = {
            ...
            if len(documents) != len(assets):
                raise Exception("Jumlah task upload tidak sesuai dengan jumlah file")

            for doc, asset in zip(documents, assets):
                upload_url = doc.get("uploadUrl")
                if not upload_url:
                    raise Exception("URL upload tidak ada")
            ...
            logger.info(f"✅ Submit dokumen selesai: {getattr(step6_data, 'get', lambda k, d=None: d)('currentStep')}")

            # Dapatkan status final (termasuk rewardCode)
...
def main():
    ...
    print("=" * 60)
    print("Tool Verifikasi Identitas Guru SheerID (Python)")
    print("=" * 60)
    print()
    ...
    if len(sys.argv) > 1:
        ...
        url = input("Masukkan link entry verifikasi SheerID (dengan externalUserId): ").strip()

    if not url:
        print("❌ Error: URL tidak ada")
        sys.exit(1)

    verification_id = SheerIDVerifier.parse_verification_id(url)
    verifier = SheerIDVerifier(url, verification_id=verification_id)

    print(f"👉 Pakai link: {verifier.install_page_url}")
    if verifier.verification_id:
        print(f"Berhasil parse verificationId: {verifier.verification_id}")
    ...
    if result.get("redirect_url"):
        print(f"URL redirect: {result['redirect_url']}")
    print("=" * 60)
