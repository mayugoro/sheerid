...
class NameGenerator:
    ...
    def _generate_component(cls, pattern):
        """Generate komponen nama berdasarkan pattern"""
        components = []
        for part in pattern:
            if part == 'prefix':
    ...
    def _format_name(cls, name):
        """Format nama (kapital huruf pertama)"""
        return name.capitalize()
    
    @classmethod
    def generate(cls):
        """
        Generate nama Inggris random
...
def generate_birth_date():
    """
    Generate tanggal lahir random (tahun 2000-2005)
