"""Modul generate bukti guru PNG - Bolt.now / PSU"""
import random
from datetime import datetime


def generate_psu_id():
    """Generate random PSU ID (9 digit)"""
...
def _get_browser_context():
    """Dapatkan atau buat browser context (singleton pattern)"""
...
def _html_to_png(html_content: str, width: int = 1200, height: int = None) -> bytes:
    """Konversi HTML ke PNG screenshot (versi optimized: reuse browser instance)"""
    try:
        context = _get_browser_context()
        page = context.new_page()

        try:
            # Set HTML content langsung, pakai domcontentloaded bukan networkidle (lebih cepat)
            page.set_content(html_content, wait_until='domcontentloaded')

            # Tunggu gambar load (jika ada gambar eksternal)
...
def generate_employment_letter_html(
    ...
    """Generate HTML surat keterangan kerja guru."""
    name = f"{first_name} {last_name}"
    now = datetime.now()
...
def _html_to_png_batch(html_list: list[tuple[str, int, int]]) -> list[bytes]:
    ...
    async def render_all():
        """Concurrent render semua gambar"""
...
def generate_images(first_name: str, last_name: str, school_id: str = '2565'):
    """
    Generate dua PNG: kartu guru + surat kerja (versi concurrent optimized)

    Args:
        first_name: Nama depan
        last_name: Nama belakang
        school_id: ID sekolah (maintain interface compatibility)
    ...
    # Concurrent generate dua gambar
    html_list = [
        (card_html, 700, 1100),
