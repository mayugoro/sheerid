...
class SheerIDVerifier:
    ...
    def verify(
        ...
        """Jalankan proses verifikasi, hapus polling status untuk mengurangi waktu"""
        try:
            ...
            # Generate PNG kartu pelajar
            logger.info("Langkah 1/4: Generate PNG kartu pelajar...")
            img_data = generate_image(first_name, last_name, school_id)
            file_size = len(img_data)
            logger.info(f"✅ Ukuran PNG: {file_size / 1024:.2f}KB")

            # Kirim info mahasiswa
            logger.info("Langkah 2/4: Kirim info mahasiswa...")
            step2_body = {
            ...
            if step2_status != 200:
                raise Exception(f"Langkah 2 gagal (status code {step2_status}): {step2_data}")
            ...
            logger.info("✅ Upload kartu pelajar berhasil")
            ...
            # Tidak melakukan polling status, langsung kembalikan menunggu review
            return {
                "success": True,
                "pending": True,
                "message": "Dokumen telah dikirim, menunggu review",
...
def main():
    ...
    if not url:
        print("❌ Error: URL tidak disediakan")
        sys.exit(1)
